...
async def create_booking(booking: BookingRequest):
    ...
    try:
        ...
        if available <= 0:
            raise HTTPException(status_code=400, detail="남은 좌석이 없습니다.")
        ...
        # 좌석번호 중복 확인 (주어진 경우)
        if booking.seat_number is not None:
            dup_resp = supabase.table("reservations").select("id").eq("route_id", route["id"]).eq("seat_number", booking.seat_number).execute()
            if dup_resp.data and len(dup_resp.data) > 0:
                raise HTTPException(status_code=400, detail="이미 선택된 좌석입니다.")
        ...
        # available_seats 감소 (간단한 낙관적 업데이트)
        try:
            supabase.table("bus_routes").update({
                "available_seats": max(0, available - 1)
